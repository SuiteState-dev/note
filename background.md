...
> **本档版本：2026-08-25 v41**（二姐第四批口径 + 法源核查轮，无代码交付）：两处，**均为方法论，且均由同一次核查暴露**。① 🔴 **新增惯例 39 —— 【已引用条款的相邻段落】是固定盲区。** 实例：我方早已引用《会计基础工作规范》**§60(七)**（借/贷/平三态规则的法源），而**紧接其后的独立一段**「现金日记帐和银行存款日记帐必须逐日结出余额」**从未进过任何一份档**，以致 `项目档 v50 §4.5.24` 写出一条**引错条号（记成 §57）、引错原文（「逐日逐笔」在全文 0 命中）、且结论方向相反**的判词。② 🔴 **新增惯例 40 —— 对照实验的前提是【对照组能产生差异】，「两组相同」先要证明数据区分得开。** 实例：金蝶日记账「显示本日小计」勾与不勾两份导出件**逐格全等**，被 `项目档 v50` 记为「三源独立互证」的第三源；实测该导出件全年**业务行仅 6 条、同账户同日多笔组合数 = 0**，且勾了的那份**「本年累计」行同样为 0** ⇒ **两个开关都不进导出件**，该件对「做不做日结」**根本没有信息量**，是 null result 被当成了 confirming result。
>
> **以下为 v40 原记：**
