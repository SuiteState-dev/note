...
> **本档版本：2026-08-25 v47**（R62 结项）：三处。① 🆕 🔴 **新增惯例 42 —— 【红线的绑定条件本身是判据的一部分；条件不触发时，正确动作是补一条护栏，不是放行】。** 实例：三护栏绑定原文是「做，但**每生成一笔分录都须满足**」，而 R62 结账向导**零分录** ⇒ **三护栏严格不触发**，可它恰是本项目第一件写 ledger 控制面的交付物。**执行方补出第四护栏而非放行。** ② 🔴 **惯例 13 补第二例 —— 【落到了 ≠ 会被读到】**：`D 选①「不建结账记录表」`四处有档、索引齐全，但 R62 工单的必读清单**一条都没指向它** ⇒ 执行方无从知道自己在推翻一条已锁定裁决，**双方当场都没发现**。🔴 **责任在下单方。** ③ 🟢 **`§2.2` 补一条兑现**：按新选活判据选中的第一件活，交付时逼出 4 个设计问题，**全部是「在 v19 上做才暴露得出来」的**。
>
> **以下为 v46 原记：**
> **本档版本：2026-08-25 v46**（Safi 拍板，**排期判据入档**）：一处，🔴 **新增 `§2.2`【l10n_cn 线的成品目标是 v20，v19 是练兵场】—— 本节是该线排期判据的唯一归宿。** 此前该判据**只存在于对话里、四份档 0 命中**，而 note 侧已按错误前提（「v19 要交付完整品」）连排两轮候选。🔴 **后果具体**：R62 原拟为「v20 迁移方案设计」，而 20.0 分支尚未切出、方案设计完须按真分支重做 ⇒ **该单据此撤销**。同时**选活判据由「v20 安不安全」改为「能不能提前暴露设计问题」**。
>
> **以下为 v45 原记：**
> **本档版本：2026-08-25 v45**（R61 结项）：一处，🔴 **惯例 24 补第六例 —— 【手段-目的错配】，一个此前没有的形状。** 前五例是：状态造不出来 / 权限不够 / 能力错配 / 环境假设本身是断言 / 结论对因果错。**第六例是：手段解决不了它被指派去解决的那个问题，而下单方没检查手段与目的对不对得上。** 实例：R61 工单要求 rebuild 以取得「账上没别人留下的东西」，而**噪声来自 demo 自带的满账余额**、rebuild 恰恰保留 demo（同一节自己也这么要求）⇒ **两句自相矛盾**；且 rebuild 是 Odoo.sh 面板动作、会切断会话，与同单「单库单会话跑完」**在 L3 手上凑不到一起**。🟢 **同时补一条【执行方侧】落法：发现工单有错时，据实报出、等下单方改，不自己绕过去。**
>
> **以下为 v44 原记：**
...
### 🆕 2.2 🔴 l10n_cn 线的排期判据：成品目标是 v20，v19 是练兵场（v46 新增，Safi 拍板 2026-08-25，**本节是该线排期判据的唯一归宿**）

> 🔴 **本节存在的理由**：这条判据此前**只在对话里，四份主档 0 命中**，以致 note 侧按错误前提（「v19 要交付一个完整品」）连排两轮候选。**新窗口若不先读本节，会立刻重犯。**

#### 一、判据本身

🔴 **`suite_cn_localization` 不在 v19 上出成品。v19 是解决设计问题的地方，v20 上只打磨一次。**

⇒ **选活的标准不是「这件事在 v20 安不安全」，是「这件事在 v19 上做，能不能提前暴露设计问题」。**

| 类别 | 处置 |
|---|---|
| **设计已无未知、只剩机械实现**，且 v20 会砸 | 🔴 **后置到 v20**。实例：**中式日记账** —— 建本键与日结两个设计问题**均已拍板**，剩下是照 SL 写；而它会给 v20 重写面加 **M-21 +6 / M-22 +1 / `cn_*` +7**（各 +37%/+33%/+27%）⇒ **现在做等于白做一遍** |
| **仍有真设计未知**，v19 上试一次能暴露 | 🟢 **现在做**。实例：**固定资产账** —— 官方 v20 已把资产报表从 custom handler + 自建 SQL 换成 **custom engine + aml shadowing**（R59-B 实证），两侧架构不同 ⇒ **钩子挂哪、覆写多深是设计问题** |
| **流程层 / 写 ledger 的东西** | 🟢🟢 **优先现在做**。理由见下 |
| **v20 破坏面本身的迁移方案设计** | 🔴 **不要在 20.0 分支切出前做** —— 靶子在动，方案做完须按真分支重做 |

🔴 **流程层为什么优先**：**流程层的设计错误是所有类别里最贵的** —— 报表错了改公式，**流程错了动客户的账**。而结账向导（lock date + 原生留痕）**几乎不碰 v20 破坏面**（不在 handler / 取数层）⇒ **同时满足「能暴露设计问题」与「v20 砸不到」**。
🟢 **兑现（R62 结项，2026-08-25）**：按这条判据选中的**第一件活**，交付时逼出 **4 个设计问题** —— ① 三护栏的绑定条件对零分录件不触发 ⇒ 立**第四护栏**；② 裁决 1 落 **(ii) 一律软锁**，推翻 note 侧预判的 (i)；③ **D 选①「不建结账记录表」收窄**；④ **软锁下被锁期间新增被静默顺延**这条已知偏离。🔴 **四条全部是「在 v19 上做才暴露得出来」的**，且**没有一条落在 v20 破坏面上**（本件 handler / 取数层命中 0）。⇒ **该判据首轮即兑现，不是纸面规则。**

#### 二、为什么这条判据成立（Safi，2026-08-25，四条）

**本模块不是急着交付的商业目的，是【战略工具】，是有客户时已储备的子弹。**

1. **用代账公司的**：只管内部账的会计用 Odoo **完全陌生**，数据呈现方式不符中国会计所需。
2. **自己报税的**：Odoo 若没有这块，用 Odoo 管业务的公司**两边录单**，重复工作量大，且要时时校验哪些单据两边对不上。
3. 🔴 **趋势判据（不是现状判据）**：金税四期后一般不做两套账；数电票三方自动对账 ⇒ **合规正从可选变必选**。
4. 🔴🔴 **最要紧的一条 —— 这是【准入】不是【份额】**：**Odoo 首先具备中国财务应有功能，才能与中国财务软件比优劣**；否则中国公司永远用中国财务软件。**必选一个软件时，缺这块就一定不是 Odoo。**

⇒ **这是 0/1 问题，不是市场份额问题。**

⚠️ 🔴 **note 侧曾用「这个客群多大」去质疑本线（2026-08-25），那是【问错了层次】** —— 市场规模是「现在卖不卖得动」的判据，**不是「该不该有这颗子弹」的判据**。该质疑已撤销，**不得再据此重排优先级**。

#### 三、三条落法

1. 🔴 **排期前先问「这件事在 v19 上做，能暴露什么设计问题」** —— 答不出来的，后置到 v20。
2. 🔴 **「v20 安全」不构成现在做的理由。** 安全只说明它不会返工，**不说明它值得占用 v19 这段时间**。（⚠️ R60/R61 的 ASBE CF 两件是按旧判据排的 —— **交付本身没问题、且已落袋，但那个理由不再适用**。）
3. 🔴 **验收阶梯是四步，别把任何一步当成"验过了"**（Safi 定，2026-08-25）：**① 我方自跑数据**（内部真值，夹具是我们造的 ⇒ 惯例 30：夹具没走到的地方绿灯什么都不证明）→ **② 二姐实操**（`observed`，单人样本）→ **③ 方案公司测试 + 查代码**（独立第二源；🔴 **其价值主要在【查代码/架构】，不在点界面** —— 该公司做一般纳税人财税一体化、不做小微，测输出反而不如二姐贴客群）→ **④ 真客户走完年终期末结转**（🟢 **唯一能证「客户能关掉金蝶」，唯一接近 `verified` 的一步**）。
   ⚠️ **② 与 ③ 各只有一次「首次接触」，都是一次性资源** ⇒ **二姐先、方案公司后**（否则第二源看到的是二姐已暴露过的问题，价值折半）；🔴 **首次接触时只记录、不提示** —— 教过之后，`§9` 指标 1（装机人工步骤数）与指标 2（首次可识别度）**永久拿不到**。
...
13. **实测得出的事实必须落主题索引，不能只落变更流水。**按时间追加的变更记录（design changelog、commit message、轮次回写）是**流水账不是索引**：下个窗口按主题找不到，只能重推一遍。
   ...
   🆕 🔴 **第二实例（R62，2026-08-25）—— 【落到了 ≠ 会被读到】，这一半此前没写。** R28-T1 定过 **D 选①「不建结账记录表」**（依据：五个 lock date 全 `tracking=True`、chatter 已记 谁/何时/旧→新），该结论**同时落在 `项目档 §5.3` / `§6.10-D` / `status §5` / `changelog v18` 四处，索引是齐的**。而 R62 工单的「必读档」清单里**一条都没指向它** ⇒ 执行方按本单判据（护栏 ③「本期是否已执行须有记录可查」）建了 `period.close.log`，**事实上推翻了一条已锁定裁决，下单方与执行方当场都没发现**。
   🔴 **责任在下单方，不在执行方** —— **执行方读不到没被指向的东西。** 惯例 13 原文管「结论要落到能查的地方」，**本例补的是下单侧**：落到了不等于会被读到。
   **落法**：出工单前，就本件的**动作对象**（本例 = 「结账留痕」）在四份档里搜一次，把命中的**已锁定裁决逐条抄进工单**，并注明「**如需推翻，报回来，不要自行改**」。
   ⚠️ **与惯例 24 的分工**：24 管**工单要求的事做不到**（手段-目的错配），本条管**工单没说的事被做反了**。
...
24. 🔴 **验收判据下发前须自查可达性 —— 尤其当本方近期交付的门控/校验可能把待观测状态变成不可构造。**
   ...
   🆕 🔴 **第六例（v45）：手段-目的错配 —— 手段解决不了它被指派去解决的那个问题。**
   前五例问的都是「这件事**做得到**吗」（状态可构造否 / 有无权限 / 有无能力 / 假设成不成立 / 因果对不对）。**第六例问的是另一个层面：这个手段，能不能达成它被写下来的那个目的？**
   **实例（R61 工单 §0.2）**：要求 **rebuild**，理由写的是「起点要求**账上没有别人留下的东西**」。🔴 **两重错**：
   ① **手段与目的错配** —— rebuild 清掉的是 R57 建的公司 9/10，而真正的噪声来自 **demo 公司自带的满账余额**（`1602`/`1122`/`2202` 等）；而**同一节又要求「rebuild 后确认 demo 仍装」** ⇒ **一边要求账上干净、一边要求保留一个带满账余额的数据集，两句自相矛盾，rebuild 达不到它被设定的目的。**
   ② **与本档 §2.1 的既有事实冲突** —— rebuild 是 **Odoo.sh 面板动作、须 Safi 做、且会切断会话**，而同一张单又写「单库单会话跑完」⇒ **这两条在 L3 手上凑不到一起。**
   🟢 **正解是执行方给的：隔离公司夹具**（测试内 `create` 全新公司 + `try_loading` + `allowed_company_ids` 锁定）。🔴 **它赢的不是便利，是证据级** —— 判据**不依赖换库状态 ⇒ 可复现、可进 CI**，而非「在某个特定库上成立」。
   **落法（下单方侧）**：🔴 **凡工单里写「用手段 X 达成状态 Y」，把 Y 单列成一句可证伪的话，再问一遍「X 之后 Y 真的成立吗」。** 手段是习惯性选的（"脏了就 rebuild"），目的是当场写的，**两者从不放在一起校对** —— 这是固定盲区。
   🟢 **落法（执行方侧，本条同样重要）**：**发现工单有错时，据实报出、等下单方改工单，【不要自己绕过去】。** 绕过去虽然本轮跑得通，但**档里那条错判据会原样留着**，下一轮换个人接手就再踩一次。⇒ 本例执行方即照此办：报出、等 L1 改单、未自行变更。
...
42. 🔴 **红线的【绑定条件】本身是判据的一部分 —— 条件不触发时，正确动作是补一条护栏，不是放行。**
   一条红线通常写成「做 X 时必须满足 A / B / C」。**下一件交付物若不做 X，A/B/C 就不触发。** 此时有两种走法：**(a)**「护栏不适用 ⇒ 可以放行」；**(b)**「护栏不适用 ⇒ 说明红线有真空，先把真空补上再放行」。
   🔴 **(a) 在形式上完全成立，而且几乎不可能被下单方当场识破 —— 因为它引的是原文。** 这正是它危险的地方：一条**看起来最守规矩**的论证，可以把风险最高的那件东西送出射程之外。
   **实例（R62，2026-08-25）**：三护栏（可预览 / 可红冲 / 可幂等）的绑定原文是「做，但**每生成一笔分录都须满足**」⇒ 护栏挂在「生成分录」上。R62 的结账向导**零分录**（只写 `res.company` 的 lock 字段），**三护栏严格不触发**；而它恰恰是本项目**第一件写 ledger 控制面**的交付物 —— 风险最高的一件，落在红线射程之外。**执行方走了 (b)**：补出**第四护栏「不可逆动作须显式二次确认、且永不【静默】施加」**，并据它重排了整件形态（结账只写软锁 / 硬锁封存独立 opt-in + 二次确认 + 滞后 N 期）。
   **落法**：
   ① 引红线放行之前，**先把绑定条件单独抄出来**，答一句「**本件触发了吗**」；
   ② 答「没触发」的，**不得就此放行** —— 要么论证本件确实不在该红线要防的风险类别里，要么**补条**；
   ③ 🔴 **下单方要防的是反向** —— 护栏是自己写的，最容易默认它覆盖一切，于是在工单里把「过三护栏」当成万能验收口。**R62 工单正是这么写的。**
   **归族**：与**惯例 31**（禁令须点名对象、不得压缩成技术手段）**互为镜像** —— 31 管**红线写窄了会堵死唯一可行路径**，本条管**红线写窄了会漏掉本该管的东西**；与**惯例 37** 相邻 —— **新立一条护栏后，须反向扫一遍已交付件**（R62 已扫出一条待裁：卸载丢分类数据 E-6）。
