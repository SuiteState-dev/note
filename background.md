...
> **本档版本：2026-08-25 v44**（R60 收窄结项）：一处。🆕 🔴 **新增惯例 41 —— 【拆轮的判据是风险分布，不是工量】**：当一轮里已验证的硬核与一处未收敛的风险绑在一起时，正确处置是**先交确证的、把风险隔离到下一轮**，而不是怼到全绿再一次性交。附一条交付伦理与一条**拆轮必附代价**的落法。实例：R60 执行方中途自陈「T2 的夹具须手工搭平，是唯一可能卡壳处」⇒ 拆 R61，R60 先交 T1。
>
> **以下为 v43 原记：**
> **本档版本：2026-08-25 v43**（R59 双单结项）：两处，🔴 **都是同一族的加重**。① 🔴🔴 **惯例 24 第五例补第二实例 —— 立条的下一轮，由立条者本人重犯。** v42 刚据 R58-A 立下「下单方给出的环境理由可以【结论对、因果错】」，而 note 侧在 **R59-B 工单**里立刻又写了一条同型的错理由：「若走原型继承，货币表迁移由官方在 v20 自行完成、我方不承接」——实测**官方 asset handler 两侧都 0 次碰货币表**（对照组：官方 TB 19.0 = 3 / master = 0），真实成因是**这条路上本来就没有货币表**。🔴 **执行方同时指出了后果**：理由错会**外推错** —— 拿它去论证日记账就会踩空。② 🔴 **惯例 35 补【没有提取方法的分母，事后连「错在哪」都追不回来】** —— M-25 分母 s-38 记 `26`、R59-A 实测 `27`，而**差额来源不可考**，因为 s-38 从未记下提取方法。
>
> **以下为 v42 原记：**
> **本档版本：2026-08-25 v42**（R58 双单结项）：四处，🔴 **全部是「数字/分母/环境假设」这一族**。① 🆕 **§7 新增【测试摘要里的 N 是收集数，不是执行数】** —— Odoo 摘要 `0 failed, 0 error(s) of N tests` 里的 N = `testsRun`，而 unittest **先 `startTest`（自增 `testsRun`）再抛 `SkipTest`** ⇒ **该数天然含跳过、且摘要行不显示 skipped**（`verified`，R58-A）。② 🔴 **惯例 35 补【整齐 ≠ 完整，整齐本身应当引起怀疑】** —— R58-B 的错正则产出 `35/35`（恰好全等、看着像干净收敛），真值是 `48/50`；**错分母比对分母更整齐**。③ 🔴 **惯例 12 补第三例：正则字符类漏一个字符 → 系统性漏掉整整一类，工具全程不吭声**（`[A-Za-z]+` 匹配不了 `Many2one` 里的数字 `2` ⇒ 全部关系字段被漏掉）。④ 🔴 **惯例 24 补第五例：下单方给出的环境理由可以【结论对、因果错】** —— R58-A 工单说「R57 那个库有真实科目表故更合适」，实测起作用的是 **demo 公司**、与 R57 发行无关。
>
> **以下为 v41 原记：**
> **本档版本：2026-08-25 v41**（二姐第四批口径 + 法源核查轮，无代码交付；**第二次推送，覆盖同号** —— 惯例 40 补【立条当日全程兑现】与一条反向教训；§7 补一条回写基建踩坑：**Markdown 里的反引号进了双引号 shell 字符串会被当命令替换静默吃掉**）：两处，**均为方法论，且均由同一次核查暴露**。① 🔴 **新增惯例 39 —— 【已引用条款的相邻段落】是固定盲区。** 实例：我方早已引用《会计基础工作规范》**§60(七)**（借/贷/平三态规则的法源），而**紧接其后的独立一段**「现金日记帐和银行存款日记帐必须逐日结出余额」**从未进过任何一份档**，以致 `项目档 v50 §4.5.24` 写出一条**引错条号（记成 §57）、引错原文（「逐日逐笔」在全文 0 命中）、且结论方向相反**的判词。② 🔴 **新增惯例 40 —— 对照实验的前提是【对照组能产生差异】，「两组相同」先要证明数据区分得开。** 实例：金蝶日记账「显示本日小计」勾与不勾两份导出件**逐格全等**，被 `项目档 v50` 记为「三源独立互证」的第三源；实测该导出件全年**业务行仅 6 条、同账户同日多笔组合数 = 0**，且勾了的那份**「本年累计」行同样为 0** ⇒ **两个开关都不进导出件**，该件对「做不做日结」**根本没有信息量**，是 null result 被当成了 confirming result。
>
> **以下为 v40 原记：**
> **本档版本：2026-08-24 v40**（R57 结项，**四处均为 note 侧自正**）：① 🔴 **§2.1 L3 行补【环境怎么运作】** —— rebuild = **同状态新库**（起始状态可再造）、**rebuild 会切断 AI 会话**（一轮必须单库单会话跑完）、**PG 用户锁死单库，L3 建不了独立库**（`verified`）。🔴 **这三条此前三份档均 0 命中**，导致 note 侧连撞两轮。② 🔴 **惯例 27 耗材判据订正**：真判据是**「一次 rebuild 能不能再造出来」**，不是「是不是单向门」。⇒ 「`zh_CN` 未激活」**不是耗材**（可再造），原文把它列为实例是**归类错误**；真耗材是**依赖已不存在的旧代码版本的存量现场**。③ 🔴 **惯例 24 补第四例：下单方对执行侧环境的假设，本身就是需要举证的断言**（note 侧在不知 PG 锁库、不知 rebuild 切会话的情况下，替 L3 排了三条路的优先级）。④ 🟢 **§7 B-73 补兑现：「统一改点分」已实测生效**（R57：两公司 claim 92 / 82，**collision 0**）。
>
> **以下为 v39 原记：**
> **本档版本：2026-08-24 v39**（R56 结项）：三处。① 🔴 **惯例 27 补【两条有效绕法】** —— 此前只列了三种**无效**绕法与「等几分钟」，**没有一条能立刻拿到真值**；现补：**① 走 git（`git fetch origin main` + `git show origin/main:<file>`，不经 raw CDN）—— 最优，不需预先知道 SHA；② raw 的 **SHA 固定路径**（`…/note/<sha>/<file>`，与分支 ref 是**不同缓存键**）。🔴 **并记一个双方各踩一次的坑：推送前先拉一次，等于把旧内容写进了自己的缓存。**② 🔴 **惯例 37 补子规则：旧结论必须记下【依据与适用条件】，否则反向扫描无法执行**（实例：`2221.08` 不预置是 2026-08-11 判过的决定，而**档里只有结论、没有二姐那句依据**，以致 R56 新事实到时判不出推没推翻）。③ 🟢 **§7 那张差集表的「官方独有 `2221.08`」补两层**：① 补登 2026-08-11 二姐判词；② 补登 R56 新事实（master 官方税有 **6 个 repartition 挂载点**指向它）。🔴 **判词：不推翻业务侧决定，但触发条件变了**（详该条）。
>
> **以下为 v38 原记：**
> **本档版本：2026-08-23 v38**（执行分工入档）：两处。① 🔴 **新增 §2.1【三层执行分工】—— 本节是「谁干什么」的唯一归宿**。此前它**在三份档里都没有 home**：`项目档 §0.1` 是**文档**职责分工（哪份档是什么），`status §11.1` 只有 R54 那次的一句临时记录。🔴 **后果已经发生**：note 侧在 R54 续跑单 #1 与 R55 工单里**两次写「分工照惯例 13」，而惯例 13 实际是「实测事实必须落主题索引」** —— 引错条号，而且因为没有正确条号可引。② 🔴 **惯例 24 补第三例：能力错配** —— 把一个**必须实跑**才能完成的判定派给**只能读源码**的一侧（B-1 `account.tax` 行排线指向）。前两例是「状态造不出来」与「权限不够」，本例是「**那侧压根没有这个能力**」。
>
> **以下为 v37 原记：**
> **本档版本：2026-08-21 v37**（R55 结项）：两处。① 🟢 **B-78 被【收窄】、不被否定** —— 两侧 `env.execute_query()` 都会自动 flush（docstring 同句），B-78 的对象**从来是 `cr.execute` 那条裸路径、不是 ORM**；若按官方新范式改用 `query.select()` + `execute_query_dict()`，自补不再需要（框架代劳）。🔴 **惯例 37 首次兑现，而且是执行方主动触发的**（上一轮才立条）。② 🔴 **新增惯例 38 —— 【迁移期陷阱】与【破坏面】是两件事，防法不同**；子规则：**兼容层只认它认识的键，不认识的静默丢弃**。实例：master `AccountReportDataCommon.__setitem__` 对非 dataclass 字段键 **no-op 不报错**，而我方 26 个 `cn_*` 键一个都不在官方 24 个字段里（我方受影面 = `status s-38 §10 M-25`）。
>
> **以下为 v36 原记：**
> **本档版本：2026-08-21 v36**（R54 收尾补正）：两处。① 🔴 **R32-T1-0-d 那条 `verified` 挂上适用版本边界** ——「`account.account.create` 无条件要求每家公司都有 `code`」**在19.0 成立、在 master 已不成立**（Check 1 全文件 0 命中）。🔴 **注意是加边界、不是降级** —— 它当时确实实测过，**变的是适用面，不是证据强度**。② 🔴 **新增惯例 37 —— 本轮查出的新事实，必须反向扫一遍旧结论**（回写只往「新增条目」走、不往「旧条是否被推翻」走，是固定盲区；并附一条子规则：**被外部变化打中的结论挂【适用边界】，不改证据级**）。⚠️ **烧新号**：v35 已被下游消费（执行方已回显），不覆盖同号。
>
> **以下为 v35 原记：**
> **本档版本：2026-08-21 v35**（R54 结项）：§7 新增 **B-81**（🔴 **`account_codes` 引擎的 `\(...)` 排除语法是【集合语义】、与顺序无关** —— 19.0 与 master 两侧正则逐字相同；以及**三条引擎正则的定义住在 community 侧**，搜 enterprise 树永远找不到）；**B-78 补一条前提消失预告**（`_init_currency_table` 在 master 上**不存在** ⇒ 该自补手段本身在 20.0 会断；`flush_all` 仍在）。
>
> §8 四处：🔴 **新增惯例 35 —— 「逐个核过 N 个」里的 N 本身要先证明完整**（分母是一个断言，不是背景；R54 三次自我推翻 5→12→23，**断的恒为同 5 条才叫收敛**）；🔴 **新增惯例 36 —— 命中差异 ≠ 成因，成因是另一次取证**（一轮内撞三次：错分母 / docstring 假阳性 / 「v20 拆了 `odoo/models.py`」）；**惯例 24 补第二例**（工单要求了执行方**权限上不可能做到**的事：note 仓库回写）；**惯例 34 补第三例**（选项集漏格：版本对不上被框成「没推 / CDN 陈旧」二选一，真实成因是**时序**）。
>
> **以下为 v34 原记：**
...
### 🆕 2.1 🔴 三层执行分工（v38 新增，**本节是「谁干什么」的唯一归宿**）

> ⚠️ **分清两件事**：`项目档 §0.1` 管的是**文档**职责（哪份档是设计权威、哪份是实现回写）；**本节管的是【人/执行侧】职责**。两者常被混用。

| 层 | 是谁 | 手上有什么 | 能产出什么 | 🔴 做不了什么 |
|---|---|---|---|---|
| **L1 note 侧** | Safi + Claude（本窗口） | 法规原件 · 金蝶原件 · 三份主档 · 二姐口径 · **note 仓库写权限** | **查资料、定设计、出工单、回写四份档** | 🔴 **无库、无 Odoo 源码树** ⇒ 不得自称实测也不得引源码行号 |
| **L2 VPS 侧** | 开发 AI 工作站 | **community + enterprise 源码树**（含 master）· 自有 `pre-workspace` | **只读源码的事实**：接口在不在 · 签名变没变 · **v20 提交方向** · 官方数据文件内容 | 🔴 **无可用库、不跑测试** ⇒ 凡需要「装上去看看」的判定一律不能派给它；🔴 **无 note 写权限** |
| **L3 开发分支** | 开发侧 | **真实 v19 库**（odoo.sh staging / dev 库）· 代码写权限 · 测试框架 | **写代码·跑测试·实跑取证**（`verified` 只能从这里来）· design 回写 | 🔴 **无 master 源码树** ⇒ v20 方向类问题不派给它；🔴 **建不了独立库**（见下） |

#### 🆕 🔴 L3 环境怎么运作（v40 补 —— 此前三份档均 **0 命中**，导致 note 侧连撞两轮）

| 事实 | 对下单的约束 |
|---|---|
| **rebuild = 同状态新库**（每次一样） | ⇒ **起始状态可再造** ⇒ 除非依赖已消失的旧代码版本，否则**不构成一次性耗材**（惯例 27 已据此订正）。**库脏了就 rebuild，不必走非提交事务 / rollback 保库** |
| 🔴 **rebuild 会切断 AI 会话**（新库要重新登录） | ⇒ **一轮任务必须在单一库、单一会话内跑完**。需要中途 rebuild 的多阶段任务，**必须拆成多张单** |
| 🔴 **PG 用户锁死在单库**（`createdb` / `CREATE DATABASE` 均 permission denied，`verified` 2026-08-24） | ⇒ **L3 建不了独立 / throwaway 库**。“另开一个一次性库”这类方案**不存在**；要独立新库只能由 Safi 在 Odoo.sh 侧 fork build |
| 库名即分支库（如 `develop-6`） | ⇒ 写回报时**直接写库名**，不写“throwaway”等不存在的对象 |

🔴 **下一轮要不要 rebuild，判据只有一句**：**下个任务的起点是否要求「某件事还没发生」？** 要求的才 rebuild（典型：验「首装 `post_init_hook` 发行」、验「`zh_CN` 未激活时的行为」）；**否则不用** —— 已发行的库有真实科目表与官方税，对多数验证任务**反而更好用**。

#### 三条铁律

1. 🔴 **下单前先问「这件事需要哪一层手上的东西」**，再决定发给谁。判别句：需要**装上去看 / 造数据 / 看报错**的 ⇒ **L3**；需要**比对两个版本的源码**的 ⇒ **L2**；需要**查法规/定口径/拍板**的 ⇒ **L1**。
2. 🔴 **一件事可以跨层，但要拆开下单，不能整块扔给一层。**实例（B-1）：「我方自发行科目与 `account.tax` 衔不衔得上」= **L2 查官方数据文件里的行排线指向哪些 `xmlid`** + **L3 装库实跑看实际落到哪个 `account_id`**，两张单。
3. 🔴 **事实归档两分法**：**Odoo 事实 → VPS 自有分册**（自有编号，不引 note 号）；**我方受影响面 → `status §10` / 主档**（note 侧写）。**两处不各记各的。**note 仓库四份档与 design 由 **L1 写入、Safi 推送**，L2 / L3 均无 note 写权限。

#### ⚠️ 引用更正

🔴 **不要再引「惯例 13」作为分工依据** —— 惯例 13 是「实测事实必须落主题索引」，与分工无关。R54 续跑单 #1 §四与 R55 工单 §一.4 均引错，**成因是当时根本没有正确条号可引**。今后引 **`background §2.1`**。
...
🔴 🆕 **适用版本边界（v36 补，R54）：下面这条只在 19.0 成立。** master（community `1d2a20a5`，2026-08-21）上 `_ensure_code_is_unique` 的**Check 1「code 必须为每个公司设置」已删除**（19.0 `:1113` 的 `ValidationError("The code must be set for every company…")` 在 master 全文件 **0 命中**；Check 2「父子公司间 code 不重复」保留）⇒ **20.0 上科目可以在某公司下无 `code`，“普通 ORM 造不出来”不再成立**。我方受影响面（TB `unmapped` 计数器新增非零来源）= `status s-37 §10 M-23`。

🔴 **不降证据级。** 本条 `verified` 是对 19.0 做的真实实测，它**当时是对的、现在在 19.0 上仍然是对的**。外部世界变了，变的是**适用面**，不是证据强度 —— 把它降成 `observed` 会把一次真实测量的价值抹掉（惯例 37 子规则）。

**这个危险状态普通 ORM 造不出来（`verified`，**19.0 限**，R32-T1-0-d）**：`account.account.create` **无条件**调 `_ensure_code_is_unique`，要求账户所属**每家公司都有 code**，否则 `ValidationError`；只有 `write` 认 `defer_account_code_checks=True` 才放行。实测：`create(company_ids=[A,B], 只给 A code)` → 报错；A 单公司带码 create + defer-write 把 B 加进 `company_ids` 且不给码 → 成功且持久。**⇒ 该状态仅经导入 / 建表模板 / load（defer 路径）产生。**
...
- **官方独有**：`2221.08` 代扣代交增值税、`2221.01.09` 进项税加计抵减 —— 后者我方一般纳税人档**缺失**。
  🆕 🔴 **`2221.08` 补两层（v39，惯例 37 子规则首次兑现）**：
  **① 旧决定的依据（此前从未落档）**：2026-08-11 二姐——「**代扣代交增值税、关税，没有境外业务都不需要**」「这两个二级科目要不要都没关系，需要的企业也可以自行增加」。⇒ **适用条件 = 目标客户无境外业务**；**重开条件 = 客户有境外业务**。（`应交关税` 同一判词，已在 `l10n_cn_assbe_chart_R33A.csv` 标「不预置」。）
  **② R56 新事实**：master 官方 `l10n_cn` 的税里，**有 6 个 repartition 挂载点指向 `2221.08`**（官方共 86 个挂载点分落四个明细科目）。
  🔴 **判词：不推翻业务侧决定，但触发条件变了。** 旧触发条件是「企业有没有境外业务」（业务层）；新增一条与业务无关的：**官方税记录本身存在时，它的 repartition 指向能不能解析**（数据完整性层）。后者 **`unknown`，需 L3 实跑**（我方自发行路径下 `2221.08` 不存在时，官方税会怎样）。**处置：维持不预置，在 L3 单里加一格验证。**

- **名称近似但不等**：我方「销项税额的抵减」vs 官方「销项税额抵减」；我方「应交所得税」vs 官方「应交企业所得税」。
- **官方数据自带毛刺**：`2221.02` 名称尾部带一个空格。⇒ **任何按名称做的同义匹配都更脆**，「判据不写同义」在此再次成立。

🆕 🟢 **兑现实测（R57，2026-08-24，`verified`）**：两家 `cn` 公司实跑发行 —— 一般纳税人档 **claim 92 / create 7 / collision 0**；小规模档 **claim 82 / create 7 / collision 0**。两档的 create 7 = **我方独有地方税号段 `2221.51–.56` 六条 + `6000`**（非 `2221` 家族），**官方 `2221.x` 全部认领**。⇒ **「点分编码下认领命中、同义重复消失」从预期变为实测**（惯例 37：不推翻、是兑现）。
...
🔴 🆕 **前提消失预告（R54，master 锚 `ddf49583`，`observed`）**：本条的自补手段 `report._init_currency_table(options)` 在 **master 上不存在** —— 连同 `_currency_table_apply_rate` / `_currency_table_aml_join` 三个同名接口全树 `def` **0 命中**。⇒ **「开第二条入口就自补」这条一般化仍成立，但在 20.0 上【要补的那个东西本身没了】**（我方受影面 = `status §10 M-21`，16 个调用点，阻断）。🟢 `self.env.flush_all()` **两侧俱在**，清单里的另一半不受影响。⚠️ **措辞限定**：名字模式全树 0 命中只能证**同名接口**没了，证不了**机制**没了（惯例 36）。

🟢 🆕 **收窄（v37，R55，`observed`）：变的是【入口】，不是 ORM。** 上面那条「前提消失预告」只说到一半 —— 实测：`env.execute_query()` 在 19.0（`self.flush_query(query)`）与 master（按 `query._sql_tuple` 元数据逐模型 `flush_model(...)`）**两侧都自动 flush**，docstring 同句。
⇒ 🔴 **B-78 的对象从来就是 `cr.execute` 那条【裸路径】** —— 我方现在走的正是裸路径（`general_ledger_handler.py:58,63` / `subsidiary_ledger_handler.py:55,59` 各自 `env.flush_all()` + `cr.execute`）。
⇒ **本条在 20.0 仍然成立**；但**若按官方新范式改用 `query.select()` + `env.execute_query_dict()`，这一轮自补就不再需要**（框架代劳）。🟢 **一般化未变，适用面变了**：“开第二条入口就要自补框架在主路径上做的准备”依然对，只是改完之后**我方不再处在第二条入口上**。
⚠️ 本条按**惯例 37** 处置：新事实反向打中旧结论时，**点回原条收窄，不另开新号**（否则同一件事两个 home）。
...
### 🆕 B-81 `account_codes` 引擎的 `\(...)` 排除语法是**集合语义**（`observed` 源码，R54）

🔴 **结论**：`5\(53, 5001, 5101, 5201)` 与 `5\(5001, 5101, 5201, 53)` **语义等价** —— 排除项是一个**集合**，不是「按序逐个减」。

**两条依据**（`_parse_account_code_engine_formulas`）：
1. `formula.replace(' ', '')` **先剥空格再 split** ⇒ `', '` 与 `','` 等价，写法差不影响解析；
2. `account['code'].startswith(excluded_prefixes)` —— `str.startswith(tuple)` 是**「任一命中即真」的集合语义**，与元组次序无关。

⚠️ **唯一的顺序敏感处**：`prefix_key = (prefix, *excluded_prefixes)` 被用作 dict 键 ⇒ 不同排列得到不同键，**但两者算出的科目集合相同，只影响分组、不影响结果**。

🟢 **两侧一致**：`ACCOUNT_CODES_ENGINE_SPLIT_REGEX`（`r"(?=[+-])"`）/ `..._TERM_REGEX`（sign / prefix / excluded_prefixes / balance_character 四组）/ `..._TAG_ID_PREFIX_REGEX` 在 19.0 与 master **逐字相同**；前缀匹配仍 `bisect + startswith(code)`。

🔴 **一条检索落法（代价已付过）**：这三条正则**定义在 community `addons/account/models/account_report.py`**，enterprise 侧是 `from odoo.addons.account.models.account_report import ...` 导入使用。⇒ **只搜 enterprise 树会得出「定义处未定位」这个假结论**（R54 实发一次）。与**惯例 15**（grep 前先确认工作树完整）同族：**15 管「树是不是整的」，本条管「搜的是不是那棵树」**。

**我方用处**：`status §10 M-20` 的 (b) 类 override（`l10n_cn_asbe_pl_cn_oc`）据此**可撤**。此前「与顺序无关」是**直觉推断**，现已换成观测。
...
12. **只读探针要先确认查的是正确载体：查到 0 ≠ 真为 0。**
   🆕 🔴 **第三例（v42）：正则字符类漏一个字符 → 系统性漏掉整整一类，而工具全程不吭声。** 提取式写作 `[A-Za-z]+`，遇到 `Many2one` / `One2many` / `Many2many` 里的**数字**即不匹配 ⇒ **全部关系字段被整类漏掉**，且既不报错也不告警，分母还显得格外整齐（详**惯例 35** v42 子规则）。
   ⇒ **与本条前两例同族**（`join` 未排序静默丢行 / `expected in xml_text` 子串假匹配）：**三例的共性都是「工具全程不吭声」**，差别只在漏的是行、是子串、还是整整一类。**落法**：凡用正则做清点，**先拿一个已知含"异类"的样本试一次**（本例：任何一个 `Many2one` 字段），确认提取式抓得到，再去跑全量。
空结果有两种成因——真的没有，或者查错了对象（错公司 / 错库 / 未装数据 / 过滤条件不同）。回滚事务里的 0 尤其骗人，因为它看起来像一次干净的实测。
...
24. 🔴 **验收判据下发前须自查可达性 —— 尤其当本方近期交付的门控/校验可能把待观测状态变成不可构造。**
   🆕 🔴 **第五例（v42）：下单方给出的环境理由可以【结论对、因果错】，而结论对会让错的因果活下来。** R58-A 工单 §0.2 写「用现有 `develop-6`，不 rebuild —— 已发行的库有真实科目表与官方税，对多数验证任务反而更好用」。实测：使那 30 处 live 前置门放行的是 **demo 公司**（`base.demo_company_cn` / `base.demo_asbe_company_cn`，`noupdate=True`，出厂即带完整科目表），`search(...limit=1)` 命中 demo 7/8 ⇒ 🔴 **R57 建的公司 9/10、`zh_CN` 激活、`coa` 发行，对结果零因果贡献**；干净 rebuild（带 demo）上同样 0 跳过。
   🆕 🔴 **第五例的第二实例（v43）—— 立条的下一轮，由立条者本人重犯，而且这次会外推。** v42 依 R58-A 立下本例后，note 侧在 **R59-B 工单 §1.1** 立刻又写了一条同型错理由：「若原型继承可行，**货币表那一族的迁移由官方在 20.0 自行完成、我方不承接**」。实测（R59-B）：官方 `account.asset.report.handler` **两侧都 0 次**碰货币表三接口，**对照组**（官方 TB：19.0 = 3 / master = 0）证明搜索方法有效 ⇒ **我方确实不承接，但成因是「这条路上本来就没有货币表」（资产按本位币计量、无折算），不是「官方代办了」。**
   🔴 **执行方同时点出了本例比 R58-A 那次更重的地方：错理由会【外推】。** 拿「官方会代办迁移」去论证**日记账**就会踩空 —— 日记账建本键是【账户 × 币别】、货币折算是核心，那条路上**没有现成官方 handler 可继承**。⇒ **错的因果不只是"没人回头查"，它会被当成通用规律用到下一个模块上。**
   ⇒ **落法补一条**：③ 🔴 **凡工单里写「因为 X 所以 Y」，把 X 单列成一行可证伪断言并【指定对照组】** —— 本例执行方正是靠一个对照组（官方 TB 也搜一遍）把「搜不到」与「本来就没有」分开的。**没有对照组的 0 命中，不能当成因果。**

   ⇒ **「不 rebuild」这个决定是对的，但下单方给的理由是错的。** 🔴 **危险在于：结论对 ⇒ 没人会回头查理由 ⇒ 错的因果被当成经验沉淀下来，下次据它做判断就会翻车。**
   **落法**：① 工单里凡写「因为环境有 X 所以能跑」，**把 X 写成一条可证伪的断言**，让执行方顺手核一下；② 执行方发现「结论对但理由错」时，🔴 **要单独报出来**，不要因为结论对就略过（本例即执行方主动报出）。
   ...
   **推论（同样重要）**：当一个状态因**本方修复**而不可构造时，正确处置是记为「**构造性关闭**」，**不是**永远挂 `observed` 待验 —— 否则下个窗口会反复尝试去验一个已经不存在的东西，而且每验一次失败都更像「这里有问题没查清」。
   🆕 🔴 **第二例（R54，2026-08-21，新形态：不是状态不可构造，是【权限上做不到】）** —— 工单要求执行方「同轮回写 design §9 + §17.8」，而 design 在 **note 仓库**，执行方**只有 `pre-workspace` 写权限**。执行方把它明说为卡点、列三条路请下单方选，**是对的做法**；若他硬凑（比如把回写内容写进自己的分册冒充交付），**就会凭空多出一份无人认领的“权威档”**。
   ⇒ **自查口径扩一条**：不只问「这个状态造得出来吗」，还要问「**这件事在他那个身份上，权限够不够**」。
   **归族**：与**惯例 29**（载体的可见人群）同形 —— 29 问「这东西送得到该看的人手里吗」，本条问「这活落在了能做的人手里吗」。

   🆕 🔴 **第三例（2026-08-23，新形态：能力错配）** —— note 侧拟将「查 `account.tax` 的 repartition line 实际落在哪个 `account_id`」发给 **VPS 侧**，而那件事**必须装库实跑**，VPS 侧只有源码树、**无可用库**。Safi 当场拦下。
   ⇒ **三例并列看得更清楚**：第一例 = **状态造不出来**；第二例 = **权限不够**；第三例 = **能力压根没有**。🔴 **自查口径扩成三问**：这个状态造得出来吗？他有权限吗？—— 以及 **他手上有做这件事需要的东西吗？**
   **预防手段**：下单前对照 **`background §2.1` 三层表的「做不了什么」列**。

   🆕 🔴 **第四例（2026-08-24，新形态：对执行侧环境的假设未举证）** —— note 侧在**不知 PG 用户锁库、不知 rebuild 会切断会话、不知 rebuild 即同状态新库**的情况下，替 L3 在三条路里排了优先级，而其中两条在它那边**根本不存在**。
   ⇒ 🔴 **假设本身就是断言（归族惯例 32）**：「他那边应该能另开一个库」与「他那边没有 X」同级，要么有档可引，要么先问。
   **预防手段**：下单前对照 **`background §2.1`**；若要引用的环境事实不在 §2.1 里，**那就是一个未举证的假设** ⇒ 先问、问完补进 §2.1。
...
27. 🔴 **施工单开头须声明「本轮需要什么起始状态」；库的某些初始状态是一次性耗材。**
   ...
   🆕 🔴 **判据订正（v40，2026-08-24）：真正的耗材判据是「一次 rebuild 能不能再造出来」，不是「是不是单向门」。**
   能再造 ⇒ **不是耗材**，放手跑、脏了 rebuild，成本只是等待时间；不能再造 ⇒ 才是耗材（典型：**依赖已不存在的旧代码版本的存量现场**，如「2.1.0 装好的库」—— rebuild 给的是当前版本，那个现场再也造不出来）。
   🔴 **下面实例①归类错了**：`zh_CN` 未激活 **rebuild 即可再造**（L3 环境事实，见 §2.1）⇒ **不属耗材**；保留该例只为记录归类错误本身。实例②（`develop-6` 已 `-u` 到 3.0.0）**才是真耗材** —— 它要的是**特定版本的存量态**。
   🔴 **代价已付**：本条前半句「用掉就没了」盖住了后半句「只能靠重建环境重新制备」，note 侧据此把 `zh_CN` 激活读成不可逆，**花两轮去解一个不存在的问题**（还替 L3 排了三条路的优先级，而其中两条在它那边根本不存在）。
   ...
   **(b) 🔴 判断 raw 拿到的是不是【陈旧副本】，看响应头，不要靠推理。**
   ...
   🆕 🟢 **两条【有效】绕法（2026-08-24 补，此前本条只列无效做法与「等一会儿」，没有一条能立刻拿真值）**：
   ① 🔴 **走 git、不走 raw（最优）**：`git fetch origin main` 后 `git show origin/main:<file>`。**它不经 raw CDN**，即刻拿到仓库真值，**不必等、也不必预先知道 SHA**。（R56 执行方实用：raw 还在给 s-39/v37 时，git 立刻给出 s-40/v38。）
   ② 🟢 **raw 的 SHA 固定路径**：`https://raw.githubusercontent.com/<org>/<repo>/<commit-sha>/<file>`。它与分支 ref 路径是**两个不同的缓存键** ⇒ 必回源（实测 `x-cache: MISS` / `source-age: 0`）。代价：得先拿到 SHA。
   🔴 **一个双方各踩一次的坑（note 侧 2026-08-23、VPS 侧 2026-08-24）：推送前先去拉了一次。**那一次回源把**旧内容**写进了 CDN，之后每次都 `HIT` 自己造的副本 ⇒ 看上去像「推送失败」。**落法：推送前不要拉；若已拉过，推送后直接用上面两条绕法核验，不要原路重试。**
...
34. 🔴 **向执行方提封闭选项题时，必须显式留「以上皆非，请说明」那一格 —— 选项集本身是下单方的一个推断。**
   ...
   **归族**：与**惯例 33** 同族但作用点不同 —— **33 管从原文推出的「结论」要挂级，本条管提问时给出的「框架」要挂级。** 两条都是「我方的推断被当成了事实」，一条发生在答，一条发生在问。
   🆕 🔴 **第三例（R54，立条那侧自犯，2026-08-21）**：执行方回报「我拉的是 v47 / s-34 / v33，工单写的是 v48 / s-35 / v34」。note 侧当场把成因框成两格：**(a) 档没推 / (b) GitHub raw CDN 陈旧副本**。🔴 **真实成因是第三格：时序** —— 那三份都是 **R54 中途回写**的，而执行方是 **R54 开工时**拉的，那会儿它们还不存在。两格里没一格对，且**两格都含「有人做错了事」的预设**，而真相是双方都没错。
   ⇒ **跨窗口 / 跨中途回写的轮次，“版本对不上”的选项集里必须常备【时序】那一格。**

35. 🔴 **「逐个核过 N 个」里的 N，本身要先证明完整 —— 分母是一个断言，不是背景。**
   🆕 🔴 **子规则（v42）：整齐 ≠ 完整，而且【整齐本身应当引起怀疑】。** 错分母常常比真分母**更好看** —— 它把不匹配的那一类整类漏掉了，剩下的自然严丝合缝。
   **实例（R58-B，执行方自纠）**：字段清点的正则写成 `[A-Za-z]+`，匹配不了 `Many2one` 里的数字 `2` ⇒ **全部关系字段被系统性漏掉**，分母报作 **`35/35` 与 `11/12`**；修正后真值是 **`48/50` 与 `13/15`**。🔴 **`35/35` 恰好全等，看上去像一次干净的收敛；真值反倒"不齐"。**
   ⇒ **落法补一条**：拿到一个「恰好全等 / 恰好整百 / 恰好没有例外」的分母时，**先反问一句「有没有一整类被我的提取式漏掉了」**，再往下用。同族见**惯例 12 第三例**（正则字符类漏字符）。
   🆕 🔴 **子规则二（v43）：没有记下提取方法的分母，事后连【错在哪】都追不回来。** 分母写进档时若只留数字、不留「怎么数出来的」，将来实测出不同的数，**你既证明不了旧数错、也定位不了差在哪一条**，只能整条重做。
   **实例（R59-A）**：`status s-38` 记 M-25 分母为 **26** 个 `cn_*` 键，R59-A 运行时提取实测 **27**（L1 独立复核逐键吻合）—— 🔴 **而差的那一个是谁，不可考**，因为 s-38 没写提取式。⚠️ 更麻烦的是：复核时才发现这 27 个**混了三种载体**（line/column 键 25 · options 键 1 · action context 键 1），⇒ **数字对不对，甚至不是最关键的问题；分母的【口径】没写才是。**
   ⇒ **落法**：① **分母与提取方法同行写**（正则/AST/SQL 原文，或至少一句「排除了什么」）；② 🔴 **分母若跨载体，必须按载体拆开列**，不得合成一个总数 —— 合成之后下游会把它当成同一类对象处理（本例：会有人拿 `AccountReportLineData.custom` 去路由一个 action context 键）。
   「N 个里逐个核过」听上去是**覆盖面证据**，实际上它是两个断言：① 那 N 个逐个核过了；② **一共就只有 N 个**。第二条往往**从未被举证**，而它才是结论成立的前提。🔴 **一个看起来严谨的错分母，比没有分母更危险** —— 它会被下游当成覆盖面证据引用。
   **实例（R54）**：“我方依赖官方方法 5 个，4 存活 1 断”—— 那个 5 只匹配了 `self._x(` / `super()._x(` 两种接收者，而我方 handler 大量用 `report._x(`。改全接收者正则 → 12；再改 **AST** → 23。**阻断从 1 条变成 5 条，结论性质随之改变。**
   **落法**：① 报「N 中的 M」时，**同句写明 N 是怎么取的**（工具 / 模式 / 扫描面）；② **显式列出该口径取不到的形状**，并逐形状说明为何可以不查；③ 换口径重取一次，看结论动不动 —— **分母变而结论不变，才叫收敛**（R54：5→12→23 三个分母下断的恒为同 5 条）。
   🔴 **取口径优先级：语法树 > 正则。** 正则分不出「这是不是代码」—— R54 里 `_should_create_account_move` 被当成真依赖挂了两轮 `unknown`，实则只存在于一句 **docstring**；AST 一换就没了。与**惯例 12 扩写**（包含≠相等）同族。
   **归族**：与**惯例 17**（计数表述须平铺、实数须誊抄）互补 —— **17 管分子怎么来的，本条管分母怎么来的。**

36. 🔴 **命中差异 ≠ 成因 —— 成因是另一次取证。**
   看到「19.0 有 / master 没有」时，大脑会立刻送上一个解释（“它被删了”“它被拆走了”“它被改名了”）。**那个解释不在观测里，是推断**，而它直接决定处置（改名 ⇒ 机械修；移除 ⇒ 重设计）。
   **R54 一轮内撞三次**：① `_field_to_sql` 报「master=0 断」，实为位置在 `odoo/orm/`（**方法在**）；② 紧接着补的成因“v20 把 `odoo/models.py` 拆成了 `odoo/orm/`”**也错** —— 19.0 早就有 `odoo/orm/`，目录重构发生在 19.0 之前；③ `_should_create_account_move` 的 0 命中成因不是“在别的模块”，是**它根本不是调用**。
   **落法**：① 命中差异只得出「两侧不同」这一句，**成因另开一次取证**（全树找同名 / 看 git 历史 / 读调用处）；② 成因写进档时必须标 `observed` 并写明**是哪一次观测得出的**（惯例 23）；③ 🔴 **单侧观测不能支撑双侧命题** —— “v20 拆了目录”是只看了 master 就下的结论，去 19.0 看一眼就倒。
   **归族**：惯例 23 管「成因要挂级」，本条管「**成因要单独取一次证**」；惯例 12 管「查到 0 ≠ 真为 0」，本条管「**0 是真的，但你给 0 配的那个故事是假的**」。

37. 🔴 **本轮查出的新事实，必须反向扫一遍旧结论 —— 回写只往「新增】走、不往「推翻】走，是一个固定盲区。**
   🆕 🔴 **第二例（v42）：同一条旧结论上，被连漏两次，而且漏的是两种不同的东西。** 「官方点分明细归档 wizard」的候选判据定在 **R35**（`85` 条 / 零分录 / 点分明细 / 其一级已被我方认领）。此后：**R43** 统一改点分、**R57** 实测「官方 `2221.x` 全部认领」⇒ **判据第三条的适用条件被改掉、分母 `85` 从未重数**（漏的是**分母**，同惯例 35）；**R56** 出 M-26「v20 税挂载点下沉到 `2221.01.01`/`.02`」⇒ **候选集合与税挂载点重叠**（漏的是**冲突**）。🔴 **四份档中「归档」与「M-26 / `2221.01.01`」此前 0 处同段共现。**
   ⇒ **落法补两条**：① 🔴 **反向扫描的对象不只是「结论」，还包括结论所依赖的【分母】与【判据的适用条件】** —— 结论本身没被推翻，但它站的地基被换了，一样不能用；② 🔴 **触发点不该只有"轮次结项"** —— 本例两次漏扫都发生在结项回写时，最后是**由 Safi 一句外行提问**（「客户自己新建的科目呢」）撞出来的。⇒ **凡判据里含【集合定义】的条目（"哪些进候选"），每次涉及该集合的新事实到来时都要点名复核一次**，不能等下一轮回写顺手带。

   一轮探查的产出天然是**增量**的（新增几条影响面、新增几个编号），而**同一批事实往往同时在底下抽掉了某条旧结论的前提**。回写时注意力全在「我要往档里加什么」，**没有任何一个环节会问「我刚才查的东西把哪条旧的干掉了」**。
   🔴 **危险度高于「漏记一条新的」：**漏记只是少一条待办（下次还能碰到）；而**一条已失效的高证据级结论躺在档里，会被下个窗口当成结论直接用**。证据级越高，危险越大 —— `verified` 恰恰是最不会被回头质疑的那一档。
   **实例（R54）**：本轮查出「官方删了 `code` 每公司必填校验」，回写时只往 `status §10` 加了一条 M 号（而且那一条还掉了），**没人去看 `background` 里那条 R32-T1-0-d 的 `verified`** —— 它写的正是「普通 ORM 造不出无 code 的科目」，**而它的失效条件恰恰就是本轮查出的那个改动**。由执行方归档时报出。
   **落法**：① 每条新事实写入前问一句「**这条事实否定了什么**」，而不只是「这条事实新增了什么」；② 拿新事实的**关键词回搜全部主档**（本例：`_ensure_code_is_unique` / 「每公司」），不只搜本轮在改的那一份；③ 找到的旧条就地挂钩，**两头都要有指针**（旧条→新 M 号，新 M 号→旧条）。
   🔴 **子规则（重要，容易做反）：被外部变化打中的结论，挂【适用边界】，不改证据级。**`verified` 记录的是「当时在那个版本上真的跑过了」，这件事**永远为真**；外部改了只说明**它管不到新版本**。把它降成 `observed`，等于把一次真实测量的价值抹掉，且下次有人在 19.0 上需要它时会以为它没验过。**正确写法 = 原级别 + 限定版本 + 失效条件 + 指向新结论。**
   🆕 🔴 **子规则（前置条件，2026-08-24 补）：旧结论必须记下【依据与适用条件】，否则本惯例根本无法执行。**反向扫描问的是「新事实有没有推翻旧结论」，而**这个问题只有在旧结论写了自己「因为什么」时才答得了**；只记结论不记依据，新事实到时只能重新拍一遍。
   **实例（R56）**：`2221.08 代扣代交增值税` 不预置是 **2026-08-11 判过的决定**（二姐：「代扣代交增值税、关税，没有境外业务都不需要」），但三份主档里**只有「不预置」这个结果（而且只写在 `应交关税` 那一行上），`2221.08` 则只作为「官方独有」的事实存在、无任何决策记录**。⇒ R56 查出 master 官方税有 6 个挂载点指向它时，**note 侧当场判不出这是不是推翻了旧决定** —— 因为旧决定没说自己管的是哪一层。
   **落法**：凡写「不做 / 不预置 / 已知偏离」，同句写清三样 —— **依据（谁说的 / 哪条法源）· 适用条件（在什么前提下成立）· 重开条件（什么变了就要重议）**。
   **归族**：与**惯例 32** 同族（「我方没有 X」与「有 X」同为断言）—— 32 管「写之前先查库里有没有」，本条管「**写之后先查库里哪条被废了**」；与**惯例 10**（跨档引用带版本号）互补 —— 10 让你**发现**旧条可能过期，本条要求你**主动去找**。
   🆕 🟢 **首次兑现（R55，立条后一轮），而且是执行方主动触发**：本轮查出「官方新取数入口自带 flush」，执行方**自行点名 B-78** 并给出了比「否定」更难的判断：**这是收窄不是否定，变的是入口、不是 ORM**，并明确要求「点回原条、不另开新号」。⇒ **本惯例的正确产出有三档：否定 / 收窄 / 挂适用边界。默认往「否定」想是错的** —— 大多数时候旧结论没错，错的是它的范围。

38. 🔴 **【迁移期陷阱】与【破坏面】是两件事，防法不同 —— 有些坑是【改了之后】才掏的。**
   升级风险清单默认回答的是「**不改直接上新版会怎样**」。但真实路径是「改完再上」，而**两条路径上的失效形态可以完全不同，甚至反向**。
   **实例（R55，M-25）**：官方把报表行/列从 `dict` 改成 dataclass。<br>• **不改直接上** ⇒ 官方对我方交的裸 dict 做属性访问 ⇒ **`AttributeError`、阻断、吻得很** ⇒ 安全（一定被发现）。<br>• 🔴 **按最自然的迁移路径改完** ⇒ `line['cn_code'] = v` **静默蒸发**，报表能跑、不报错、中式列全空 ⇒ 危险。<br>   ⇒ **危险度随迁移上升**。把它当成普通破坏面登记，会在「改完了、不报错了」那一刻被当成已解决。
   **落法**：① 风险清单每一条要分两问 —— 「**不改会怎样**」与「**按推荐改法改完会怎样**」；② 第二问答案含「静默」的，**迁移工单里必须先写往返断言**（写进去→读出来，惯例 30 的注入→检出形状），**写在改造代码之前**；③ 登记时两种形态分开写，不压成一条。
   🔴 **子规则：兼容层只认它认识的键，不认识的静默丢弃。** 官方提供兼容层（本例：`__getitem__` / `__setitem__`）时，它覆盖的是**官方自己的字段集**，**不覆盖你挂在上面的自定义键**。⇒ 凡往官方对象上挂自定义键的，**升级时先问「官方给自定义键留了正门没有」**（本例留了：`custom` 字段 + `from_dict` / `update_values` / `as_dict`），**而兼容层往往不走那扇正门** —— 这正是静默失效的成因。
   **归族**：与**惯例 30** 同族（测试通过 ≠ 被测路径执行过）—— 30 管「断言没跑到」，本条管「**写入根本没发生**」；与**惯例 37** 互补 —— 37 管「新事实废了哪条旧结论」，本条管「**新事实在什么时点才会发作**」。

39. 🔴 **【已引用条款的相邻段落】是固定盲区 —— 引用一条法源/接口时，把它的上下文一并读完再走。**
   查证时的检索单位是**要找的那一条**；一旦命中，注意力立刻转向「怎么用它」，**紧挨着它的下一段就再也不会被看见**。这与「材料不在库里」（惯例 32）**不是同一件事，也更隐蔽** —— 材料不但在库里，而且**已经被引用过、被写进档、被当成权威源反复援引**。
   **实例（2026-08-25）**：我方自 `项目档 v17` 起即援引《会计基础工作规范》**§60(七)**「凡需要结出余额的帐户……没有余额的写『平』」作为中式账表方向栏三态的法源，此后 R48 / R49 / R50 / R51 多轮引用。而**同属 §60、就在 (七) 正下方的独立一段**——「**现金日记帐和银行存款日记帐必须逐日结出余额**」——**在四份主档中 0 命中**。后果是 `项目档 v50 §4.5.24 一` 写下一条三重错误的判词：**条号错**（记成 §57，而 §57 讲的是订本式账簿）、**原文错**（「逐日逐笔」六字在整部法源 **0 命中**）、🔴 **结论方向相反**（拿「逐笔登记 ≠ 逐日结余」去论证「按月出余额与法源不冲突」，而法源恰恰是**直接命令逐日结余**）。
   ⚠️ **注意该判词自己写了「本档首次写入，待反驳」** —— 标注了不确定性，**但没有触发任何复核动作**。⇒ **「待反驳」不是防线，它只是一张欠条**；写下它的同时必须写清**由谁、拿什么去反驳**（本例：原件就在 `legal documents/` 里，一次 grep 的事）。
   **落法**：① **凡从法源/官方源码引出结论，取证时连带读完【所在条款的完整范围】**（一条的全部款项、一个方法的全部 docstring 与相邻方法），不止命中的那一句；② **引用必带可复核坐标**（条号 + 原文片段），使下一个人能一次 grep 定位 —— 本例正因为写了条号才被查出错；③ 🔴 **凡在档里写「原文是……」，回写前必须在原件上 grep 一次那几个字** —— 「逐日逐笔」若被 grep 过一次，这条错误当场就死了。
   **归族**：与**惯例 32**（说「我们没有 X」与说「有 X」同为断言）相邻但更狠 —— 32 管「没查」，本条管「**查了、命中了、然后停在命中处**」；与**惯例 33** 互补 —— 33 管「从法源推出的『因此』不得压过原件」，本条管「**法源原文本身没读全**」。

40. 🔴 **对照实验的前提是【对照组能产生差异】—— 「两组结果相同」在数据区分不开时，不是结论，是哑。**
   拿「开关 A 打开 / 关闭各导一份，两份一样 ⇒ 开关 A 无效果」这类推理时，**默认了一个未经检验的前提：如果开关真有效果，这份数据能把它显示出来**。前提不成立时，「两份一样」既不支持也不反对任何结论，**它只是没有信息量**；而它在回写时极易被当成一条独立的佐证源计入。
   **实例（2026-08-25）**：`项目档 v50 §4.5.24 一` 把「日结还是月结」记为「**三源独立互证**」，第三源是「导出件勾与不勾逐格全等」。实测该导出件：242 行 = 表头 1 + 期初 18 + 本期合计 216 + **业务行 6**，**同账户同日出现多笔的组合数 = 0** ⇒ 每日至多一笔时，日小计行与业务行**天然不可区分**。更直接的一刀：勾选方同时勾了「显示本年累计」，而该份导出件里**「本年累计」行数 = 0** ⇒ **两个开关都不写进导出件**（只影响屏幕），该件对本问题**结构性沉默**。⇒ 三源实为**两源**，且余下两源讲的都是**产品行为与实务习惯**，无一源在回答**合规**问题。
   **落法**：① 记「A 与 B 无差异」之前，先答一句「**如果有差异，这份数据显示得出来吗**」，答不出就标 `unknown`、不计为源；② 🔴 **计数源的时候，null result 不占一个源位**；③ 对照件要**挑得出差异的样本**上取（本例：现库已有同日两笔的账户，重导一次即可），不在稀疏演示数据上取。
   **归族**：与**惯例 30**（测试通过 ≠ 被测路径执行过）同构 —— 30 是「断言没跑到那条路径」，本条是「**数据没走到那个分支**」，两者都表现为**绿灯 / 一致**；与**惯例 35** 互补 —— 35 管「分母要证明自己完整」，本条管「**对照组要证明自己有分辨力**」；与**惯例 36** 互为镜像 —— 36 说「有差异 ≠ 知道成因」，本条说「**无差异 ≠ 得到结论**」。
   🟢🟢 **立条当日即兑现，且走完了全程（2026-08-25）**：识别哑证 → 按落法③ 指出「在有同日多笔的账户上重导」→ 五分钟后取到判别样本 → **判别力具备（同账户同日多笔 = 2）而日小计行仍为 0** ⇒ **哑证转真结论**。🔴 **此处有一条比本条本身更值钱的观察**：真结论与原哑证**方向相同**（都指向「没有日小计行」），⇒ **如果当时不追这一步，会得到一个【正确的结论 + 无效的证据链】** —— 而下一次同样的省略未必还这么运气好。**本条防的不是错结论，是错证据；错证据的代价要到很久以后才付。**
   ⚠️ **同轮还带出一条反向教训**：判别样本到手后，原本被它支持的方案（补日小计行）**反而失去了产品对位物** —— 原以为「与厂商同构」，实测厂商并不出这一行。⇒ **取证补上之后，要回头重跑一遍【当初基于哑证做的所有权衡】，不只是改那一格结论**（同惯例 37 的形状）。


### 🆕 测试计数：摘要里的 N 是【收集数】，不是【执行数】（v42，`verified`，R58-A）

🔴 **Odoo 测试摘要行 `0 failed, 0 error(s) of N tests` 里的 N = `testsRun`，而 `testsRun` 在 `startTest` 时就自增了。** unittest 的顺序是**先 `startTest`、再抛 `SkipTest`** ⇒ **N 天然把被跳过的用例算在内，而摘要行又不显示 `skipped`。**
⇒ 🔴 **「N tests 0 failed」这句话在结构上无法区分「N 条都跑了」与「N 条里有一半跳了」。**

**实例（R58-A，`suite_cn_statement`）**：历轮引用的 `122 / 94 / 89 / 68 / 63` 全部是这个 N。本轮实跑逐用例解析日志（`OdooTestResult`，`odoo/tests/result.py`：`startTest`→`INFO Starting <test>`；`addSkip`→`INFO skipped <test>: <reason>`；失败→`FAIL`/`ERROR`；**成功不打印**）得：**执行 = Starting − skipped，跳过 = skipped，失败 = FAIL + ERROR**。结果 `122 全执行 / 0 跳过 / 0 failed` ⇒ 🟢 **本轮二者相等，但那是这个 DB 状态下的相等，不是 N 自身保证的。**

🔴 **连带一条更狠的**：R46 那套「空绿排查」的判据是「把被测方法体换成 `return`/`[]`，还绿即空绿」——**对被跳过的用例结构性失明**（skip 在调用任何东西之前就发生，方法体换成什么都照样"绿"）。⇒ **「空绿残留 0」这类结论，只覆盖【跑到了的】那部分。**

**落法**：① 🔴 **回报测试结果一律给三数并相加对齐**（执行 / 跳过 / 失败 = 总数），**不得只报 `0 failed`**；② 三数从**逐用例日志**誊抄，不从摘要行抄；③ 若「收集 = 执行」，🔴 **写明它成立的环境条件**（本例：装 demo + `xlrd` 在场 + 夹具在库），**不得把它当成代码属性外推到别的库**；④ 历史轮次各自 DB 的跳过态**已不可追**，一律标 `unknown`，**不追溯复核**（库已不存在，属惯例 24 的「构造性关闭」）。
**归族**：与**惯例 30**（测试通过 ≠ 被测路径执行过）同族 —— 30 管「断言没跑到」，本条管「**用例根本没开始**」，而后者连打桩都照不出来。

### 🆕 回写基建：反引号进 shell 会被静默吃掉（v41，`verified`）

🔴 **现象**：用 `python3 -c "..."`（**双引号**）跑回写替换时，字符串里的 Markdown 行内代码标记（反引号）会被 shell 当成**命令替换**执行，结果是那一段被**替换成命令的输出（通常是空）**，而 Python 与 `assert` **全部照常通过** —— 因为断言查的是替换前的锚，不是替换后的产物。
**实例（2026-08-25）**：一次内联回写把四份档的版本行里 5 处行内代码引用吃空（`§4.5.24 一-bis` / `observed` / `ca_journal_rpt` 等），`stderr` 只留下 `§4.5.24: not found` 这类看起来像噪声的行；**四个 sha256 照常打印、断言 8/8 全绿**。
🔴 **危险点在于它与惯例 30 同形**：绿灯是真的，只是绿错了地方 —— 断言证明了「锚被找到并替换」，**没有证明「替换进去的内容与我写的一致」**。
**落法**：① 🔴 **回写一律走脚本文件，不用内联 `-c`**；② 若必须内联，用**单引号**包外层，或让 Python 从 `chr(96)` 构造反引号，**绝不让反引号出现在双引号 shell 字符串里**；③ 🔴 **回写后的验收必须回读产物、比对关键片段**，不能只看断言通过与哈希打印 —— **哈希只证明「文件变了」，不证明「变成了我要的样子」**；④ `stderr` 非空一律当成事故信号，哪怕主流程返回 0。
**归族**：与**惯例 30**（测试通过 ≠ 被测路径执行过）同构；与 **惯例 12**「凡用『包含』代替『相等』的判定其计数不作数」同族 —— 本例是**断言对象选错了**：该断言的是产物，断的却是输入。
41. 🔴 **拆轮的判据是【风险分布】，不是【工量】—— 已验证的硬核不该陪未收敛的风险一起悬着。**
   一轮做大了要不要拆，直觉判据是「工量大不大」。**那个判据是错的。** 正确的问法是：**这一轮里，风险是均摊的，还是集中在一两处？** 若集中，则把**已经验证成立的部分**压在同一轮里等它，等于**拿确定的东西去赌不确定的**——那部分的价值不会因为多等一轮而增加，却会因为同轮翻车而一起退回。
   **实例（R60，2026-08-25）**：一轮含 T1 正表（35 行）+ T2 间接法（25 行）+ 跨表判据。执行方在 T1 全绿后**中途停下自陈**：T2 的科目映射是推演，且跨表判据要求夹具**账账勾稽平、须手工搭平**，「**是唯一可能卡壳的地方**」。⇒ 风险**不是均摊的**，集中在 T2 的夹具上。**处置：R60 收窄、先交 T1；T2 拆入 R61，风险隔离在自己那一轮。**
   🟢 **同轮记一句执行方原话，它是这条惯例的行为面**：「**与其怼出个半绿 sprawl 或谎报全绿，我先把确证的交给你**」。⇒ **中途如实报进度并停下，优于凑一个看起来完整的交付。**
   🔴 **落法（拆轮必附代价，否则拆轮本身会制造盲区）**：
   ① **拆之前先问「风险是均摊还是集中」**，集中才拆；均摊则拆了也没隔离效果，只是多一轮开销。
   ② 🔴 **拆轮必须同时登记【被拆走的判据留下了什么覆盖缺口】。** 实例：跨表判据「间接法净额 == 正表经营净额」随 T2 走了 ⇒ **R60 交付的正表，经营活动净额只由表内勾稽验过、无第二条独立路径对过**。这条已落 design + 测试 docstring。🔴 **不写下来，下个窗口会以为它验过了** —— 拆轮制造的盲区，和漏测制造的盲区，长得一模一样。
   ③ **被拆走的那一轮，工单里要写明它同时负责闭合上一轮的哪条边界**，形成闭环而不是两条平行线。
   **归族**：与**惯例 30**（测试通过 ≠ 被测路径执行过）互补 —— 30 管「验了但没验到」，本条管「**没验，而且因为拆轮所以看起来像验了**」；与**惯例 37** 相邻 —— 37 管新事实反扫旧结论，本条管**拆轮时旧轮的判据归属要重算**。
