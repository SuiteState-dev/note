...
14. **一次验收里若有强弱两组证据，弱的那组必须自带标签。**当某组测试**按设计必然通过**（改动在该场景下不产生差异、或该维度的值恰好为 0），它的「全绿」不构成任何正确性证据；而回报里两组并列时，读者默认它们等权。
   ...
   **它仍有价值，但价值是「同源一致性检验」，不是「互证」。** 真互证需要**一条独立取数路径**（如另写一段直接从 `account.move.line` 聚合的对照）。
   **落法**：措辞里出现「互证 / 交叉验证 / 两个来源对上了」之前，先答一句「**这两个来源共用了哪一段代码**」。共用了，就把措辞降到「一致性检验」。**下单方尤其要防** —— 这条错的措辞是写在施工单里由下单方带进来的，执行方照着抄了一遍。
