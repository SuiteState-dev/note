...
**B-67 `post_init_hook` 不随 `-u` 重跑 ⇒ 对官方记录的外科覆盖在升级路径上不重放（19.0，`observed`，2026-08-11）**
...
**等级 `observed`**：dev 库单点观察，未跑受控用例（装旧版 → `-u` → 查公式）。**接客户前须实测确认。**

---

**B-68 翻译型字段在 create 时冻结，后装语言不回溯（19.0，`observed`+，2026-08-13）**

`account.account.name` 一类翻译字段以 jsonb 按语言键存储。**记录创建那一刻，写入哪些键由当时的环境决定**；此后激活新语言，**不会回头补写已存在的记录**。

**同一根因下已观察到三个症状**：

| # | 症状 | 观察者 |
|---|---|---|
| 1 | **先装科目表、后加中文语言 ⇒ 中文环境里科目名永久显示英文** | Safi，多次遇到，一直未修 |
| 2 | 我方 `post_init_hook` 在 `lang` 未设（默认 `en_US`）的环境跑 ⇒ **中文名写进 `en_US` 键**（`{"en_US": "未交增值税"}`） | R35 实测 |
| 3 | 同一份数据经 `post_init` 与 wizard 两条路径 ⇒ **落库结果不同**（一条只有 `en_US`、一条 `en_US`+`zh_CN` 都是中文） | R35 实测 |

🔴 **症状 1 不是我方缺陷，官方 chart 同样中招** —— 它是这一类的通例，不是个例。⇒ **判缺陷归属前，先问「官方自己的数据在同样顺序下会不会一样坏」。**

🔴 **隐蔽性**：中文界面下 `zh_CN` 缺失会**回落显示 `en_US`**。若 `en_US` 里装的是中文，界面看起来完全正常，**肉眼查不出**；只有显式取 `zh_CN` 的地方（导出模板、打印模板）才会空出来。

**三条做法**：

1. **装机顺序写进实施手册**：**先装语言，再建公司/装科目表。** 这是顺序依赖，不是偏好。
2. **自己发数据时，语言键要显式写，不依赖 `env.lang`。** 依赖环境 = 落库结果不确定。
3. **验收要显式取键，不能看界面。** 界面回落会掩盖问题；查 jsonb 原值才算数。

**未决**：目标语言尚未激活时能否直接写该语言键（`with_context(lang=...)` 是否落到目标键、还是回落 `en_US`），`unknown`，须实测。这决定做法 2 是「加前置校验」还是「直接写双键」。
